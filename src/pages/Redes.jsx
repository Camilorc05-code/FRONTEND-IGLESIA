...
function ReelPreview({ reel, platform, color }) {
  ...
  const fbId = platform === 'facebook' ? extractFBVideoId(reel.url) : null;
  const igId = platform === 'instagram' ? extractIGReelId(reel.url) : null;
          ...
          {!playing ? (
            <>
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-16 h-16 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center border border-white/30 group-hover:bg-white/30 transition-all duration-300">
            ...
            <>
              {platform === 'facebook' && fbId && (
                <iframe
                  ref={iframeRef}
                  src={`https://www.facebook.com/plugins/video.php?href=https://www.facebook.com/reel/${fbId}&show_text=false&width=400&autoplay=1`}
              ...
              {platform === 'instagram' && igId && (
                <iframe
                  ref={iframeRef}
                  src={`https://www.instagram.com/reel/${igId}/embed/?autoplay=1`}
                  className="absolute inset-0 w-full h-full border-0"
                  allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; screen-wake-lock"
          ...
          <div className="absolute bottom-0 left-0 right-0 p-3 pointer-events-none">
            <div className="flex items-center gap-2">
              <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0" style={{ background: color }}>
                {platform === 'facebook' ? (
                  <svg className="w-3 h-3 text-white" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                ) : (
              ...
              <span className="text-white text-xs font-medium drop-shadow-lg truncate">
...
function PlatformSection({ title, subtitle, icon, color, gradient, reels, platform, url, btnLabel, btnColor }) {
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-5 md:px-8 py-16 md:py-24 relative z-10">
        ...
        <StaggerGroup className="grid grid-cols-2 sm:grid-cols-3 gap-4 md:gap-6" stagger={0.1}>
          {reels.map((reel) => (
            <ReelPreview key={reel.id} reel={reel} color={color} platform={platform} />
          ))}
        </StaggerGroup>
      </div>
    </section>
  ...
  return (
    <div>
      ...
      <PlatformSection
        ...
        icon={<svg className="w-6 h-6" viewBox="0 0 24 24" fill="none"><defs><linearGradient id="igS2" x1="0" y1="24" x2="24" y2="0"><stop offset="0%" stopColor="#F56040"/><stop offset="50%" stopColor="#C13584"/><stop offset="100%" stopColor="#833AB4"/></linearGradient></defs><rect x="2" y="2" width="20" height="20" rx="5" stroke="url(#igS2)" strokeWidth="2"/><circle cx="12" cy="12" r="5" stroke="url(#igS2)" strokeWidth="2"/><circle cx="17.5" cy="6.5" r="1.5" fill="url(#igS2)"/></svg>}
        reels={reelsIG}
        platform="instagram"
        url={IG_URL}
      ...
      <section className="bg-paper2">
        <div className="max-w-3xl mx-auto px-5 md:px-8 py-16 text-center">
          <Reveal>
            ...
            <p className="text-ink/50 max-w-lg mx-auto mb-10">
